...
namespace :rewards do
  desc "Gives bonuses when someone has a birthday"
  task :give_birthday_bonus do
    RewardsBirthday.create_birthday_bonuses
    Snitcher.snitch(ENV["SNITCH_DAILY"]) if ENV["SNITCH_DAILY"]
  end

  desc "Gives birthday bonuses for people by emails passed as args"
  task :give_birthday_bonuses_to, [:white_list] do |_task, args|
